...
def ResNet18(conv_layer, linear_layer, init_type, num_classes, pretrained=False, **kwargs):
    # assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    model = ResNet(conv_layer, linear_layer, Bottleneck, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']), strict=False)
    fc_features = model.fc.in_features
    model.fc = linear_layer(fc_features, num_classes)
    return model


def ResNet50(conv_layer, linear_layer, init_type, num_classes, pretrained=False, **kwargs):
    # assert init_type == "kaiming_normal", "only supporting default init for Resnets"
...
def ResNet101(conv_layer, linear_layer, init_type, num_classes, pretrained=False, **kwargs):
    # assert init_type == "kaiming_normal", "only supporting default init for Resnets"
    model = ResNet(conv_layer, linear_layer, Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
...
if __name__ == '__main__':
    ...
    y = net(Variable(torch.randn(2, 3, 112, 112)))
